...
/// 玩家的反制是手枪：命中扣血（默认 3 枪死）+ 反向冲量 + 全腿松手（墙/顶上会掉下来）；
/// 血量归零倒地蜷缩，静止后尸体永久冻结。1/2/3 切换蜘蛛预设，R 重开。
///
/// 玩法全部在宿主层；内核只走 opt-in 接缝（BeginLeap / Launch / Conscious + Leaping 观测），
/// 回归路线零漂移。相位机见 <see cref="ArenaPhase"/>。
/// 无头自检：<c>--bot=still|strafe --ticks=N [--preset=name] [--shoot-every=N]</c>
/// 用脚本化玩家跑固定 tick 数，结尾打 <c>[SPIDER-ARENA-RESULT]</c>。
/// </summary>
public partial class SpiderArenaWorld : Node3D
{
    ...
    /// + 重力回归——墙/顶上必然掉下来，地面上被推退一截再抓地。</summary>
    [Export(PropertyHint.Range, "0,20,0.25")]
    public float HitImpulseMps { get; set; } = 4.0f;

    [Export(PropertyHint.Range, "0,10,0.25")]
    public float HitImpulseUpMps { get; set; } = 1.5f;
    ...
    public override void _Ready()
    {
        ...
        GD.Print($"[SPIDER-ARENA] ready preset={_breed.Name} tps={HostPhysicsTps} " +
                 ...
                 $"hp={SpiderHp} bot={_bot} botTicks={_botTicks} " +
    ...
    private void LandPounce()
    {
        ...
        _controller.BeginLeap(rebound * (ReboundSpeedMps * (float)TickDt), reboundTicks);
    ...
    private void HandleHit(string part, Vector3 gunDirection)
    {
        ...
        string fall = surface == SpiderSurfaceKind.Ground ? "" : " (knocked off)";
    ...
    private void BotTick()
    {
        ...
        if (_botShootEvery > 0 && _tick % _botShootEvery == 0 && _phase != ArenaPhase.Dead)
        {
    ...
    {
        if (_screenshotsTaken.Contains(name))
            return;
    ...
    private void ProcessPendingScreenshot()
    {
        ...
        if (_pendingShotStage == 0)
        {
            ...
            if (_controller.SupportNormal.Y < -0.5f)
    ...
    private void BotFinish()
    {
        ...
        bool pass = _leapAttempts >= 1 && (_surfaceTicks[1] + _surfaceTicks[2]) > 0
            && (_botShootEvery > 0 ? _phase == ArenaPhase.Dead : _pounceCount >= 1);
