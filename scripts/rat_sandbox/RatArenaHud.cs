...
/// <summary>
/// 鼠煞枪击竞技场 HUD：准心十字（Grabbed 期隐藏）+ 命中 toast + 左上状态行（F1 显隐）+
/// 中央大字提示（GRABBED / BITTEN ×n）+ 底部常驻小字（R = RESTART）。
/// 纯展示——不持有任何玩法状态，世界脚本每帧推送（toast TTL 也归世界脚本管）。
///
/// 沿用仓库 HUD 纪律（DaddyLongLegsGrabHud 骨架）：代码构建、无 Theme 资源、逐控件覆盖；
/// 只用 Label/ColorRect/PanelContainer（天然不可聚焦）且全部 <c>MouseFilter = Ignore</c>，
/// 保证鼠标捕获与按键永远不会被 UI 吞掉。
/// </summary>
public sealed class RatArenaHud
{
    ...
    {
        _layer = new CanvasLayer { Name = "RatArenaHud" };
    ...
    /// <summary>命中 toast（TTL 由世界脚本管理，过期传空串隐藏）。</summary>
