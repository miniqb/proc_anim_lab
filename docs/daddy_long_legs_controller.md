...
**追补修复（同日第二轮，用户实机反馈）：多断手后取手死锁**。症状：连断多根后怪物「想接手
却停在原地蠕动，永远接不上」。**被推翻的初判**（headless 实测否决）：① 多残肢运动力崩溃
——5 残肢近全速（0.03~0.074 m/tick）、7 残肢半速（0.02~0.035）走完 17m，运动力无恙，蠕动
只短暂出现在多根同时痛缩垂吊的窗口（`dirSup=0`、身体沉地）；② 牵引超时活锁——牵引一旦
启动 3~14 tick 即接合，身体速度远追不过 0.075 m/tick 的断口伺服（评审工作流独立复现同一
结论）。**真实根因是三环死锁**：牵引启动要求残肢 `CanAcceptExternalTarget`，其中
`NeededForLocomotion` 是粘性门——职责分配器只自主释放支撑贡献**最差**的腿，抓稳在断口旁的
残肢贡献高永远轮不到；该断落段因此始终「可尝试」，fetch 每 tick 选最近断口 → 怪物永久停在
断手正上方原地踏步，其余断手全部饿死。修复三件套：① 内核 opt-in
`TryReleaseTentacleForExternalUse`（语义见回迁契约 §4.1d；首版「配额 ≤ 下限即拒绝」实测仍
死锁——稳态配额恰好常驻下限（loco=3），改为按 `AssignBestIdle` 同款守卫预判补位候选，分配器
的「低于下限立即补位」路径有冷却豁免、缺口下 tick 自动补上）；② 宿主 `ReattachTick` 断口
进半径且残肢不可用时显式请求让位；③ 让位持续失败（`ReattachBusyWaitSeconds(2s)` 容忍）则按
重试冷却临时降权该断落段先接别的，防单点挟持。验证：远断手场景（7 断手、断落段距身体
15~21m）修前 fetch 停在 P8 上 500+ tick 零牵引，修后走回去 7/7 全部接回（t=1090~1276，零降
权事件）；近断手场景与修前逐 tick 一致（141/151/181/192/212 起牵引，让位路径未被触发）；
默认败北链逐 tick 一致；`core/smoke` + daddy smoke（哈希 `47F9584427FCD54A` 不变）+ 40 项
矩阵全绿。同轮顺修（评审工作流发现）：**正在伸抓玩家的残肢中弹曾是无声 no-op**——抓取与
痛缩共用 `PlayerTargetId`（§7.2 同 ID 接管设计），`_grabTentacle` 不清则 `FeedGrabSnapshot`
每 tick 把玩家目标喂回去，痛缩下一 tick 被 grab 冲突分支判成 Detached，伸抓照旧毫无反馈。
修复：失能分支命中 `_grabTentacle` 时先 `ClearExternalTarget` + 清抓取 + 抓取重试冷却（镜像
断手分支；束缚期开枪被相位门封死，该分支只在 Chase 伸抓途中可达）。验证：12 段残肢 t=290
伸抓、t=291 中弹 → `shot disabled` + 痛缩完整 2s（无 `flinch detached`）→ t=372 重新伸抓
→ t=403 抓住（永不放弃抓玩家不变）。
...
**追补调整（同日，用户需求）**：**挣脱连打期镜头震动**。玩家相机新增化妆偏移通道
（`SetCameraShake`：局部位移 + 欧拉增量，叠在 pitch 之上；`Yaw`/`CameraPitch` 语义不变，
但 Eye* 系读数会跟着抖）。世界脚本 `UpdateCameraShake` 每渲染帧驱动：**目标包络只在
`Bound && _struggleActive` 时为 1**——刚被抓的扭头对准（挣扎未激活）与挣脱失败被拖走
（Dragging）都不震（用户显式要求）；包络快起（60ms）慢收（120ms，硬切画面会跳，被拖走
时的残余在 ~0.3s 内消失）；每次猛拽瞬间叠一记 ×`StruggleShakeTugKick` 的快衰减冲量强化
「激烈感」。噪声用五条不可通约频率的正弦叠加（1/0.61/0.83/0.47/0.73×基频）——无 RNG、
纯渲染侧、不进物理与哈希。调参：`Arena / Struggle` 组新增 `StruggleShakeDegrees
(1.5°，0=关)`、`StruggleShakeOffsetMeters(0.03)`、`StruggleShakeFrequencyHz(11)`、
`StruggleShakeTugKick(2.2)`。

对抗评审轮（3 视角 + 逐条对抗复核）命中 3 个真问题，全部已修：① **拖拽物理吃到震动位移**
——初版 `EyePosition` 读相机节点全局位置，挣扎失败进 Dragging 时包络还有 ~0.83s 收敛尾巴，
`DragTick` 的吞没判定/拖拽方向随渲染帧率漂移，违反纯渲染侧约束。根治：玩家 `EyePosition /
EyeForward / EyeBasis` 全部改从未震动的 yaw/pitch 刚体推导（震动只落在相机节点这最后一层，
任何逻辑观测量都读不到），无震动时与旧算法逐位等价（败北链 tick 复验一致）。② **合成俯仰
可翻过竖直极点**——pitch 夹 ±1.35 rad 后才加震动分量，且猛拽冲量无界累积，合法调参极端组合
（Degrees=6 + TugKick=6 + 密集猛拽）下相机 X 会越过 90°画面倒置。修：冲量封顶
`ShakeKickMax=3` + 相机合成俯仰硬夹 1.5 rad（<π/2）双保险。③ **残余震动漏进下一轮扭头窗口**
——`RegrabGraceSeconds` 调小到 <0.9s 时快速再抓，上一轮包络还没收敛完。修：`EnterBound`
硬清包络/冲量（此刻残余极小，硬切不可见）。headless 验证：默认链自动对准并激活挣扎，震动
路径被真实驱动（含三次猛拽冲量），修复前后败北链 tick 都与基线一致、零报错。
