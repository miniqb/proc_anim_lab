...
>
> 当前状态：**M0 项目章程**（只有目标 + 参考文档，尚未写任何系统代码）。
...
6. **locomotion 模式靠涌现**：抓着可达地形 → **重力关成 0**（不会掉墙）；入水 → 腿垂下（Dangle）；走/爬在腿与身体两层**都无分支**，只有"抓住/没抓住"这一个开关。玩家式**显式状态机**仅在需要精细操控的角色上才用。
...
| **M3** | 地形涌现：斜坡 / 墙 / 水 → 走爬游三态自然涌现（重力开关 + 射线方向切换） | 待办 |
| **M4** | 多样化与调参：多足 / 尾巴 / 多种体型，参数化手感（对标 `LizardBreedParams`） | 待办 |
| **M5** | 移植接口：抽出与引擎解耦的模块，定义回迁 `random-room-runtime` 的边界 | 待办 |

> 下一步（M1 开工前）：初始化 Godot C# 项目骨架（`project.godot` / `.csproj` / 一个测试场景）。当前 M0 刻意**不建**，先把起点与目标钉清楚。
...
- 本项目为**独立 git 仓库**（已 `git init`，尚未提交）。
