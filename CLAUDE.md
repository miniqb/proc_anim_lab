...
>
> 当前状态：**M6 Cicada 独立后端完成（2026-07-30）**——`ProcAnim.Core` 现在包含 Lizard / Cicada 两个平行的物种控制器；蝉已覆盖 3D 自由飞行、悬停、显式三面停驻、起飞、Charge、四翼四触须和 light/dark 预设，并有独立无引擎 smoke 与 Godot 矩阵。M5 的内核抽离、回迁契约和 Lizard 外部评审修复仍保持完成；「默认集成姿态」的闭环在主仓接线后验证（契约 §4.1/§8.3）。
>
